...
pid_t Scheduler::addProcess(const Process * pr, const Frequency freq)
{
	Prcocesses[freq].push_back(pr);
	pid_t pid;
	pid.first = freq;
	pid.second = Prcocesses[freq].end();
}
...
void Scheduler::update()
{
	...
	if (time - last_call_time() > 10) // 10 milliseconds 
	{
		...
		if (_0.1hr_count == 1000) // 1000/100  = 0.1
		{
			for (auto i : processes[_0.1hr])
			{
				(*i)->update();	
			}
			_0.1hr_count = 0;
